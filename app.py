...
<style>
    ...
    .metric-card { background: linear-gradient(135deg, #1a0a0a, #2e0d0d);
    ...
    .fraud-result { background: linear-gradient(135deg, #3e0000, #7f0000);
        border: 2px solid #ef5350; border-radius: 12px;
        padding: 24px; color: white; text-align: center; }
    .legit-result { background: linear-gradient(135deg, #003300, #1b5e20);
        border: 2px solid #66bb6a; border-radius: 12px;
        padding: 24px; color: white; text-align: center; }
    .scenario-card { background: #1a0a0a; border: 1px solid #c62828;
...
with st.sidebar:
    ...
    st.markdown("<div style='color:#ffebee;font-size:0.85rem;margin-top:4px;'><b>Scenario 3</b>: Customer compromised (14 days, 5× amounts)</div>", unsafe_allow_html=True)
    st.markdown("---")
...
if page == "Predict":
    ...
    if submit:
        if model is None or scaler is None:
            ...
            st.markdown("---")
            st.markdown(f"#### {icon(ICO_TARGET, 20, '#ffebee', '#c62828')} Fraud Risk Meter", unsafe_allow_html=True)
            ...
            st.markdown(f"#### {icon(ICO_SHIELD, 20, '#ffebee', '#c62828')} Risk Factor Analysis", unsafe_allow_html=True)
            factors = []
            if tx_amount > 220:
            ...
            for title, detail, color in factors:
                st.markdown(f"<div style='border-left:4px solid {color};padding:8px 14px;background:#1a0a0a;border-radius:4px;margin:4px 0;'><b>{title}</b><br><span style='color:#aaa;font-size:0.85rem;'>{detail}</span></div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    if uploaded_csv is not None:
        ...
        if missing:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f'<div class="metric-card"><h2>{fraud_count}</h2><p>Fraudulent</p></div>', unsafe_allow_html=True)
            with col2:
                st.markdown(f'<div class="metric-card"><h2>{total-fraud_count}</h2><p>Legitimate</p></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"### {icon(ICO_TARGET, 20, '#ffebee', '#c62828')} Objective", unsafe_allow_html=True)
        st.markdown("""
        Build a system that classifies whether a **financial transaction is fraudulent or legitimate**
        based on transaction features and behavioural patterns.
        Credit card fraud costs the global economy billions annually. Automated detection systems
        are critical for real-time transaction monitoring.
        """)
        st.markdown(f"### {icon(ICO_LAYERS, 20, '#ffebee', '#c62828')} Dataset Details", unsafe_allow_html=True)
        st.markdown("""
        | Property | Details |
        |---|---|
        ...
        """)
    with col2:
        st.markdown(f"### {icon(ICO_ALERT, 20, '#ffebee', '#c62828')} 3 Fraud Scenarios Simulated", unsafe_allow_html=True)
        for title, desc in [
            ...
            st.markdown(f'<div class="scenario-card"><b>{title}</b><br><span style="font-size:0.9rem;">{desc}</span></div>', unsafe_allow_html=True)

    st.markdown("---")
    st.markdown(f"### {icon(ICO_CPU, 20, '#ffebee', '#c62828')} Methodology", unsafe_allow_html=True)
    st.markdown("""
...
# ══════════════════════════════════════════════════════════
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"### {icon(ICO_CPU, 20, '#ffebee', '#c62828')} Model Details", unsafe_allow_html=True)
        st.markdown("""
        | Property | Details |
        |---|---|
        ...
        """)
        st.markdown(f"### {icon(ICO_SHIELD, 20, '#ffebee', '#c62828')} Why Precision & Recall Matter", unsafe_allow_html=True)
        st.markdown("""
        ...
        """)
    with col2:
        st.markdown(f"### {icon(ICO_CHART, 20, '#ffebee', '#c62828')} Feature Importance", unsafe_allow_html=True)
